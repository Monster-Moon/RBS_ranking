...
readLines('../soi/ED-00028-00000010.soi', n = 11)
apa_data = read.table('../soi/ED-00028-00000010.soi', skip = 11)
apa_data_list = apply(apa_data, 1, function(x) unlist(strsplit(x, ',')))
apa_data = apa_data_list[unlist(lapply(apa_data_list, length)) == 6] %>% do.call('rbind', .) %>% as.matrix()

apa_data = matrix(as.numeric(apa_data), nrow = nrow(apa_data), ncol = ncol(apa_data))

apa_data_list = lapply(1:nrow(apa_data), function(i)
  apa_data[rep(i, apa_data[i, 1]), -1])

apa_data = apa_data_list %>% do.call('rbind', .) %>% as.matrix()
apa_data = apa_data + 1
rownames(apa_data) = colnames(apa_data) = NULL
head(apa_data)
...
{
  set.seed(i)
  split_sample = sample(1:3, size = nrow(apa_data), prob=c(0.7,0.15,0.15), replace = TRUE)
  train = apa_data[split_sample == 1, ] %>% as.matrix()
  valid = apa_data[split_sample == 2,] %>% as.matrix()
  test =  apa_data[split_sample == 3,] %>% as.matrix()
  data_list[[i]] = list(train = train, valid = valid, test = test)
  cat(i, '\n')
}
save(data_list, file = 'r3_data_list.Rdata')

#### model fitting ####
setwd('/Users/moon/Documents/CBS_model/code/real_data/soc_code')
load('r3_data_list.Rdata')
source('../model.R')
...
save(model_list, file = 'r3_model_list.Rdata')
...
save(btm_model_list, file = 'r3_btm_list.Rdata')
...
save(bt_model_list, file = 'r3_bt_list.Rdata')
...
source('..//model.R')
load('r3_data_list.Rdata')
load('r3_model_list.Rdata')
load('r3_btm_list.Rdata')
load('r3_bt_list.Rdata')
...
r3_result = list(train = train, test = test, valid = valid)
save(r3_result, file = 'r3_result.Rdata')

...
{
  set.seed(i)
  split_sample = sample(1:3, size = nrow(mtdot_data), prob=c(0.7,0.15,0.15), replace = TRUE)
  train = mtdot_data[split_sample == 1, ] %>% as.matrix()
  valid = mtdot_data[split_sample == 2,] %>% as.matrix()
  test =  mtdot_data[split_sample == 3,] %>% as.matrix()
  data_list[[i]] = list(train = train, valid = valid, test = test)
  cat(i, '\n')
}
save(data_list, file = 'r1_data_list.Rdata')
...
save(model_list, file = 'r1_model_list.Rdata')
...
save(btm_model_list, file = 'r1_btm_list.Rdata')
...
save(bt_model_list, file = 'r1_bt_list.Rdata')
...
r1_result = list(train = train, test = test, valid = valid)
save(r1_result, file = 'r1_result.Rdata')
